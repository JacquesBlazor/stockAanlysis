# author: alvin.constantine@outlook.com, datetime: 2021/3/1 23:00pm, License: GNU 3.0 License
...
class security_crawler:
    def __init__(self):
        ...
        self.tradedaytoday = self._get_tradedaytoday()
    ...
    def _get_tseallbut0999(self, url, accquiredfile):
        try:
            df = pd.read_html(StringIO(requests.get(url,
                headers=self.twse_headers).text), encoding='utf-8')[8]
        except Exception as e:
    ...
    def _get_otcstkquote(self, url, accquiredfile):
        try:
            df = pd.read_html(StringIO(requests.get(url,
                headers=self.tpex_headers).text), encoding='utf-8')[0]
        except Exception as e:
            print('從網頁讀取 %s 資料時發生錯誤。錯誤訊息: %s' % (accquiredfile, e))
            return None
        try:
    ...
    def _get_t187ap03L(self, url, accquiredfile):
        try:
            resp = requests.get(url, headers=self.twse_headers)
            resp.encoding = 'utf-8'
            df = pd.read_csv(StringIO(resp.text), encoding='utf-8', usecols=[1,2,3,4,5,7,14,15,16,27,31],
    ...
    def _get_t51sb11(self, url, accquiredfile):
        try:
            resp = requests.post(url, headers=self.twse_headers, data={'firstin': 'true', 'step': '10', 'filename': 't51sb11.csv'})
            resp.encoding = 'dbcs'
            df = pd.read_csv(StringIO(resp.text), encoding='utf-8', usecols=[0,1,2,4,5,12,14,15],
